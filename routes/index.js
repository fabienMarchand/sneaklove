...
router.get("/sneakers/:cat", async (req, res) => {
  ...
  tags = await Tag.find({});
  res.render("products.hbs", { sneakers, tags, scripts: ["client.js"] });
});
...
router.get("/myfilteredShoe/", async (req, res) => {
  const objTagId = req.query.tags;

  sneakers = await Sneaker.find({ id_tags: objTagId });
  res.json(sneakers);
});
